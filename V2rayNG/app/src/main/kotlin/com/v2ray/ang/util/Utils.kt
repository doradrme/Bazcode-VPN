...
object Utils {
    ...
    fun getUrlContentWithCustomUserAgent(urlStr: String?): String {
        ...
        conn.setRequestProperty("Connection", "close")
        conn.setRequestProperty("User-agent", "Bazcode/${BuildConfig.VERSION_NAME}")
        url.userInfo?.let {
            conn.setRequestProperty("Authorization",
                "Basic ${encode(urlDecode(it))}")
        }
        conn.useCaches = false
        // Trust all SSL certificates for old Android compatibility
        if (conn is HttpsURLConnection) {
            try {
                ...
                conn.sslSocketFactory = sc.socketFactory
                conn.hostnameVerifier = HostnameVerifier { _, _ -> true }
            } catch (e: Exception) { /* use default SSL */ }
        }
