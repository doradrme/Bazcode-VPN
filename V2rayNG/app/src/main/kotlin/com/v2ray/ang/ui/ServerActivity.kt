...
class ServerActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when(config?.configType ?: createConfigType) {
            ...
            EConfigType.VLESS -> setContentView(R.layout.activity_server_vmess)
            EConfigType.TROJAN -> setContentView(R.layout.activity_server_vmess)
        }
    ...
    private fun saveVnext(vnext: V2rayConfig.OutboundBean.OutSettingsBean.VnextBean, port: Int, config: ServerConfig) {
        ...
        if (config.configType == EConfigType.VMESS) {
            ...
            // Preserve existing flow for VLESS (no UI spinner for flow editing)
        }
    }
