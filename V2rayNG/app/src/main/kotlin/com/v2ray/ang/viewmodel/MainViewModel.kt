...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun sortByTcping() {
        serverList.sortWith(compareBy<String> {
            val d = MmkvManager.decodeServerAffiliationInfo(it)?.testDelayMillis ?: 0L
    ...
    fun testServerTcping(guid: String) {
        val outbound = MmkvManager.decodeServerConfig(guid)?.getProxyOutbound() ?: return
        val host = outbound.getServerAddress() ?: return
        val port = outbound.getServerPort() ?: return
        tcpingTestScope.launch {
            val result = Utils.tcping(host, port)
