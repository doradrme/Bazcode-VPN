    ...
    private fun importConfig(str: String?, subid: String, removedSelectedServer: ServerConfig?): Int {
        try {
            ...
            if (str.startsWith(EConfigType.VMESS.protocolScheme)) {
                ...
                val hashPos = str.indexOf('#')
                val baseUri = if (hashPos >= 0) str.substring(0, hashPos) else str
                val remark = if (hashPos >= 0) Utils.urlDecode(str.substring(hashPos + 1)) else ""
                val uri = URI(baseUri)
                val queryParam = (uri.rawQuery ?: "").split("&")
                    .mapNotNull { p ->
                        val idx = p.indexOf('=')
                        if (idx <= 0) null else p.substring(0, idx) to Utils.urlDecode(p.substring(idx + 1))
                    }.toMap()
                ...
                val security = queryParam["security"] ?: ""
                if (security.equals("reality", true)) {
                    streamSetting.populateRealitySettings(
                        serverName = queryParam["sni"] ?: sni,
                        fingerprint = queryParam["fp"] ?: "chrome",
                        publicKey = queryParam["pbk"] ?: "",
                        shortId = queryParam["sid"] ?: "",
                        spiderX = queryParam["spx"] ?: ""
                    )
                } else {
                    streamSetting.populateTlsSettings(security, allowInsecure, queryParam["sni"] ?: sni)
                }
            }
            if (config == null){
                return R.string.toast_incorrect_protocol
        ...
        try {
            ...
            return config.configType.protocolScheme + when (config.configType) {
                ...
                EConfigType.VLESS -> {
                    ...
                    if (streamSetting.security.equals("reality", true)) {
                        streamSetting.realitySettings?.let { r ->
                            if (!TextUtils.isEmpty(r.serverName)) dicQuery["sni"] = r.serverName
                            if (!TextUtils.isEmpty(r.fingerprint)) dicQuery["fp"] = r.fingerprint
                            if (!TextUtils.isEmpty(r.publicKey)) dicQuery["pbk"] = r.publicKey
                            if (!TextUtils.isEmpty(r.shortId)) dicQuery["sid"] = r.shortId
                            if (!TextUtils.isEmpty(r.spiderX)) dicQuery["spx"] = r.spiderX
                        }
                    } else {
